...
const { JWT_SECRET, SENDGRID_API_TOKEN } = process.env;
...
const register = async (req, res, next) => {
  ...
  try {
    sendGrid.setApiKey(SENDGRID_API_TOKEN);
...
const resendVerificationEmail = async (req, res, next) => {
  ...
  try {
    ...
    const sendMail = {
      ...
      text: "Confirm your email address12312412",
      html: `<strong>Confirm your account by following this link
<a href = "http://localhost:3000/api/users/verify/${user.verificationToken}">Please click on this link to confirm your email</a>
</strong>`,
    };
    const response = await sendGrid.send(sendMail);
    console.log(response);
